...
var data = input.Split("\n")
    ...
    .Select(game => new
    {
        GameId = int.Parse(game[0].Split(" ")[1]),
        Sections = game[1].Split("; ")
            ...
            .Select(section => new
            {
                Red = section.Where(color => color.Color == "red").FirstOrDefault()?.Amount ?? 0,
                Green = section.Where(color => color.Color == "green").FirstOrDefault()?.Amount ?? 0,
                Blue = section.Where(color => color.Color == "blue").FirstOrDefault()?.Amount ?? 0
            })
    })
    .ToArray();
...
WriteLine("Part 1:");
WriteLine(JsonSerializer.Serialize(x, new JsonSerializerOptions { WriteIndented = true }));
